class LateralSimplesRule:
    """Classe com as regras para calcular Lateral Simples"""
    ...
    CAMPOS_NECESSARIOS = [
        {
            ...
            'required': True,
            'min': 1,
            'help': 'Quantas laterais simples você precisa'
        },
        {
            'name': 'altura',
            'label': 'Altura (cm)',
    ...
    def calcular(dados, componente):
        """
        Calcula a quantidade de material necessária para laterais simples
        ...
        if componente.tipo_componente.codigo == 'AC-001':  # MDF
            ...
            area_por_lateral = altura_m * largura_m
            ...
            return {
                ...
                'unidade': 'm²',
                'resumo': f'{quantidade}x laterais de {altura}cm x {largura}cm = {area_total:.4f} m²'
            }
        
        elif componente.tipo_componente.codigo == 'AC-002':  # FITA
            # Calcular perímetro para fitas de borda
            perimetro_por_lateral = 2 * (altura_m + largura_m)
            perimetro_total = perimetro_por_lateral * quantidade
            
            return {
                'sucesso': True,
                'perimetro_por_lateral': perimetro_por_lateral,
                'perimetro_total': perimetro_total,
                'quantidade_utilizada': perimetro_total,
                'unidade': 'm',
                'resumo': f'{quantidade}x laterais = {perimetro_total:.2f}m de fita'
            }
        
        else:
            return {'erro': 'Componente não compatível com laterais simples'}